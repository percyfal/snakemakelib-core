...
def number_of_detected_genes(infile, cutoff=1.0, quantifier="TPM", **kwargs):
    """Aggregate expression data frame to count number of detected genes

    Args:
      infile (str): csv file name with expression values
      cutoff (float): cutoff for detected gene
      quantifier (str): quantifier label; usually TPM or FPKM

    Returns:
      detected_genes (DataFrame): aggregated data fram with number of detected genes per sample
    """
    expr_long = read_gene_expression(infile)
    expr_long[quantifier] = [math.log2(x+1.0) for x in expr_long[quantifier]]
    try:
