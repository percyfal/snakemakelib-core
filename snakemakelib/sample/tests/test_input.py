...
import csv
import pytest
from ..input import parse_sampleinfo, samples_from_input_files
...
class TestParseSampleinfo:
    def test_parse_sampleinfo(self):
        samples = parse_sampleinfo(sampleinfo=csv.DictReader(['SM\n', 'Sample1\n', 'Sample2\n']))
        assert samples == [{'SM':'Sample1'}, {'SM':'Sample2'}]

    def test_parse_sampleinfo_samplecolumn(self):
        samples = parse_sampleinfo(sampleinfo=csv.DictReader(['Sample\n', 'Sample1\n', 'Sample2\n']),
...
class TestSamplesFromInputFiles:
    def test_samples_from_input_files(self, illumina_scilife_files, mocker):
        ...
        samples = samples_from_input_files(config['settings']['sample_organization'].run_id_re)
        assert samples[0] == {'SM': 'P001_102', 'DT': '120924', 'PU2': '2', 'PU': 'AC003CCCXX_2', 'PU1': 'AC003CCCXX', 'PATH': 'P001_102/120924_AC003CCCXX'}
